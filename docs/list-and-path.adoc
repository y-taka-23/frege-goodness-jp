...
おや、面白いことが見て取れますね。*それぞれの関数は要素のリストを返し、その要素の一つ一つが次の関数の入力になっています*。ということは、このパターンを一般化し、「<<easy-io.adoc#easy-io,お手軽入出力>>」でやったように _bind_ 関数を使用することができそうです。
...
嬉しいことに、すでに _bind_ 関数が使える形になっていて、「<<easy-io.adoc#easy-io,お手軽入出力>>」と同じように `>>=` で記述することができます。
